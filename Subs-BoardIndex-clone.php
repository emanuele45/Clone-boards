...
function getBoardIndex($boardIndexOptions)
{
	...
	$result_boards = $smcFunc['db_query']('boardindex_fetch_boards', '
		SELECT' . ($boardIndexOptions['include_categories'] ? '
			...
			b.id_board, b.name AS board_name, b.description, b.child_level,
			CASE WHEN b.redirect != {string:blank_string} THEN 1 ELSE 0 END AS is_redirect,
			CASE WHEN b.clone = {int:zero} THEN 0 ELSE b.clone END AS is_clone,
		...
		array(
			...
			'child_level' => 0/*$boardIndexOptions['base_level']*/,
			'blank_string' => '',
			'zero' => 0,
		)
	);
	...
	{
		if (!empty($row_board['is_clone']))
		{
			$clones[] = array(
				'id_cat' => !empty($row_board['id_cat']) ? $row_board['id_cat'] : 0,
				'id_board' => $row_board['id_board'],
				'id_parent' => $row_board['id_parent'],
				'clone_of' => $row_board['is_clone'],
			);
		}
		$seeks[$row_board['id_board']] = $seek++;

		if (empty($boardIndexOptions['countChildPosts']))
		{
			if (!((!empty($boardIndexOptions['base_level']) && ($row_board['child_level'] >= $boardIndexOptions['base_level'] && $row_board['child_level'] <= 1 + $boardIndexOptions['base_level'])) || (empty($boardIndexOptions['base_level']) && $row_board['child_level'] >= $boardIndexOptions['base_level'])))
				continue;
		}
		...
		if (structureBoard($row_board, $boardIndexOptions, $this_category, $isChild))
		{
			$latest_post = prepareBoard($row_board, $this_category, $isChild, $boardIndexOptions, $latest_post, $ignoreThisBoard);
			$latest_board = $row_board['id_board'];
		}
	}
	...
	if (!empty($clones))
		...
		{
// 		_debug($clone['clone_of']);
			if (!isset($seeks[$clone['clone_of']]))
			 continue;

			$smcFunc['db_data_seek']($result_boards, $seeks[$clone['clone_of']]);
			$row_board = $smcFunc['db_fetch_assoc']($result_boards);
			$row_board['id_cat'] = $clone['id_cat'];
			$row_board['id_parent'] = $clone['id_parent'];
			$row_board['id_board'] = $clone['id_board'];

			if (structureBoard($row_board, $boardIndexOptions, $this_category, $isChild, true))
			{
				if ($latest_board < $row_board['id_board'])
				{
					$latest_post = prepareBoard($row_board, $this_category, $isChild, $boardIndexOptions, $latest_post, $ignoreThisBoard);
					$latest_board = $row_board['id_board'];
				}
				else
					prepareBoard($row_board, $this_category, $isChild, $boardIndexOptions, $latest_post, $ignoreThisBoard);
			}
		}
...
function structureBoard($row_board, $boardIndexOptions, &$this_category, &$isChild, $forceOverwrite = false)
{/*_debug($boardIndexOptions['include_categories'] ? 1 : 2);*/
	// This is a parent board.
	if ($row_board['id_parent'] == $boardIndexOptions['parent_id'])
	{
		// Not a child.
		$isChild = false;
		fillBaseBoardInfo($row_board, $this_category, $forceOverwrite);
		return true;
	}
	// Found a child board.... make sure we've found its parent and the child hasn't been set already.
	elseif (isset($this_category[$row_board['id_parent']]['children']) && (!isset($this_category[$row_board['id_parent']]['children'][$row_board['id_board']]) || !$boardIndexOptions['include_categories']))
	{
		// A valid child!
		$isChild = true;
		fillChildBoardInfo($row_board, $this_category);
		return true;
	}
...
function prepareBoard($row_board, &$this_category, $isChild, $boardIndexOptions, $latest_post, $ignoreThisBoard)
{
...
function fillBaseBoardInfo($row_board, &$this_category, $forceOverwrite = false)
{
	global $user_info, $scripturl, $txt;

	// Is this a new board, or just another moderator?
	if (!isset($this_category[$row_board['id_board']]) || $forceOverwrite)
	{
...
function fillChildBoardInfo($row_board, &$this_category, $ignoreClone = false)
{
	global $user_info, $scripturl, $txt;
